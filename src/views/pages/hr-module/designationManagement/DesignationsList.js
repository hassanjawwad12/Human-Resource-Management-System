...
const DesignationsList = () => {
  ...
  const getDesignations = useSelector((state) => state.designationReducer.designationList);

  const [rows, setRows] = React.useState(getDesignations);
  const [search, setSearch] = React.useState('');

  React.useEffect(() => {
    setRows(getDesignations);
  }, [getDesignations]);

  const handleSearch = (event) => {
    const filteredRows = getDesignations.filter((row) => {
      return (
      ...
      <Box>
        <div className="flex flex-row items-center w-full px-4 justify-between ">
          ...
          <div className="flex flex-row gap-2 items-center">
            ...
            />
            <AddDesignationModal count={count} setCount={setCount} />
          </div>
        </div>

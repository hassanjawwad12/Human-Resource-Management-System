...
import {
  decideLeaveRequest,
  getPendingLeaveQuotaByCompanyId,
...
const Allowed = ({ count, setCount, flag, data }) => {
  ...
  const ApproveLeave = () => {
    ...
    formData.append('employeeLeaveId', data.companyEmployeeLeaveId);
    formData.append('isApproved', flag);
    formData.append('managerRemarks', remark);

    dispatch(decideLeaveRequest(formData))
      .then((result) => {
        if (result.payload.SUCCESS === 1) {
          setAlert({
            open: true,
  ...
  return (
    <div>
      <Tooltip title={flag === 1 ? 'Approve Leave Request' : 'Reject Leave Request'}>
        <IconButton
          onClick={handleDialogOpen}
          sx={{ color: flag !== 1 ? 'red' : 'primary.main' }}
          size="small"
        >
          {flag === 1 ? <AddTaskIcon fontSize="small" /> : <DeleteIcon fontSize="small" />}
        </IconButton>
      </Tooltip>
      <Dialog open={open} onClose={handleDialogClose} maxWidth="md" fullWidth>
        <DialogTitle sx={{ bgcolor: 'primary.main', py: 2 }}>
          <Typography variant="h4" className="text-white">
            {flag === 1 ? 'Approve Leave' : 'Reject Leave'}
          </Typography>
        </DialogTitle>
        <DialogContent className="h-full p-6">
          {data && (
            <div className="flex flex-col items-start justify-center w-full h-full gap-6 mt-4">
              {/* Avatar and Employee Details */}
        ...
        <DialogActions sx={{ display: 'flex', justifyContent: 'space-between', px: 3, py: 2 }}>
          ...
          <Button onClick={ApproveLeave} variant="outlined" sx={{ color: 'white' }}>
            Confirm
          </Button>
        </DialogActions>
      </Dialog>
